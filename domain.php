<?php
	...
	if(empty($_SESSION["user_token"]) && empty($_SESSION["user_id"]) || !isset($_GET["fbid"]))
	{
		?>
			<script>
				window.location="./";
	...
	if(!$d)
	{
		//$query = $conn->query("INSERT INTO `flame_records_b1`.`users` (`id`, `name`, `email`, `user_id`) VALUES (NULL, '".$uname."', '".$uemail."', '".$uid."');");
		//$d = mysqli_fetch_array($query, MYSQLI_ASSOC);
	}	
...
<html>
	<head>
		<title>Domain | Pagecabinet</title>
		...
		<!-- Banner 
			...
			<section id="one" class="wrapper style1" style="background:#f9f9f9; height: ;">
					<div style="width: 80%; margin-left: auto; margin-right: auto;">
						...
						<form action="./orderplaceonshelf.php" method="post" autocomplete="off">
							<?php 
								$check = 0;
								for($x=0; $x < $feed_count; ++$x)
								{
									...
									if($id == $_GET["fbid"])
									{
										echo '
										<tr>
											<td><strong>Your Page: </strong></td>
											<input type="hidden" name="pid" value="'.$id.'"
											<!--td  style="background-image: url(\''.$bgpic.'\'); width: auto; background-repeat:no-repeat; background-size: auto 100%;"></td-->
											<td><a href="./getIdRedirect.php?fbid='.$id.'" target="blank"><img src="./images/eye_watch.png" style="height:24px; width:27px; vertical-align:center;" /></a> <strong>'.$name.'</strong></td>
										</tr>
									';
									$check = 1;
									}
							...
							?>
								<tr>
									<td><strong>*Your Domain: </strong><br><font size="1"><li><a href="http://www.1and1.com/domain-names" target="blank">I do not have a domain</a></font></td>
									<td><input type="text" name="domain" placeholder="example.com" required/></td>
								</tr>
								<tr>
									<td><strong>*Email: </strong></td>
									<td><input type="email" name="email" value="<?php echo $uemail; ?>" placeholder="someone@example.com" required/></td>
								</tr>
								<tr>
								...
								<tr>
									<td><strong></strong></td>
									<td><input type="submit" name="sbt" value="Order Now" /></td>
								</tr>
